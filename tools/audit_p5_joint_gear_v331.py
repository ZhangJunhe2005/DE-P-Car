#!/usr/bin/env python3
"""Independently audit the V3.3.1 unilateral-safe-bank correction."""
...
def main(argv=None):
    ...
    config, config_sha, v33_config, v31_config, base_config, acceptance = trainer.load_config()
    ...
    if formal and checkpoint != expected:
        raise RuntimeError("formal V3.3.1 audit requires canonical best checkpoint")
    output_path = trainer.resolve(args.output) if args.output else (
        checkpoint.with_suffix(".acceptance.json") if formal
        else ROOT / "reports/p5_joint_gear_v331_pilot_acceptance.json"
    )
    _bundle_path, bundle, sequence_path, _authority, data_gate = (
        trainer.v33.v31.v3.verify_data_authority(base_config)
    )
    ...
    required = {
        ...
        "modality": "fusion", "artifact_role": "best", "run_completed": True,
    ...
    errors = [
        "checkpoint_" + key for key, expected_value in required.items()
    ...
    if (
        ...
        or contract.get("candidate_and_score_frozen") is not True
        or contract.get("production_qualified") is not False
    ):
        errors.append("checkpoint_contract")
    if errors:
        raise RuntimeError("V3.3.1 artifact identity failed: " + ",".join(errors))
    ...
    metrics = trainer.v33.epoch_loop(
    ...
    checks = trainer.v33.acceptance_checks(metrics, acceptance)
    passed = all(checks.values())
    report = {
        "schema": "DEPCarJointGearV331AcceptanceV1",
        ...
        "training_config": str(trainer.CONFIG), "training_config_sha256": config_sha,
        "trainer_sha256": trainer.sha256_file(trainer.TRAINER),
        "acceptance_contract": str(trainer.resolve(config["base_contract"]["acceptance_contract"])),
        "acceptance_contract_sha256": config["base_contract"]["acceptance_contract_sha256"],
        "candidate_and_score_frozen": True,
        "metrics": metrics,
