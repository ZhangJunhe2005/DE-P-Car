#!/usr/bin/env python3
"""Independently audit the V3.1 sequence-correction checkpoint."""
...
import train_dep_car_sequence_v31 as trainer
...
def main(argv=None):
    ...
    config, config_sha, base_config, acceptance_contract = trainer.load_config()
    ...
    if formal and checkpoint != expected:
        raise RuntimeError("formal V3.1 audit requires the canonical best checkpoint")
    if args.output:
        output_path = trainer.resolve(args.output)
    elif formal:
        output_path = checkpoint.with_suffix(".acceptance.json")
    else:
        output_path = ROOT / "reports/p5_joint_gear_v31_pilot_acceptance.json"
    ...
    if not contract_path.is_file():
        raise RuntimeError("V3.1 checkpoint contract is missing")
    ...
    if errors:
        raise RuntimeError("V3.1 artifact identity failed: " + ",".join(errors))
    ...
    checks = trainer.sequence_acceptance_checks(metrics, acceptance_contract)
    passed = all(checks.values())
    report = {
        "schema": "DEPCarJointGearV31AcceptanceV1",
        "status": (
