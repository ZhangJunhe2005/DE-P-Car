...
TRAINER="$PROJECT_ROOT/tools/train_dep_car_sequence_v32.py"
AUDITOR="$PROJECT_ROOT/tools/audit_p5_joint_gear_v32.py"
MODEL_ROOT="$PROJECT_ROOT/models/dep_car/p5_joint_gear_v32"
LOG_ROOT="$PROJECT_ROOT/logs/p5_joint_gear_v32"
REPORT_ROOT="$PROJECT_ROOT/reports"
FORMAL_OUTPUT="$MODEL_ROOT/fusion_winner_sequence_correction.pth"
PILOT_OUTPUT="$MODEL_ROOT/pilot/fusion_winner_sequence_correction_lowdim_v2.pth"
...
usage() {
  echo "Usage: bash scripts/run_p5_joint_gear_v32.sh --stage STAGE [options]"
...
if [[ "$stage" == "dry-run" ]]; then
  "$PYTHON" "$TRAINER" --output "$FORMAL_OUTPUT" \
    ...
    | tee "$REPORT_ROOT/p5_joint_gear_v32_dry_run.json"
  exit "${PIPESTATUS[0]}"
fi

if [[ "$stage" == "pilot" ]]; then
  "$PYTHON" "$TRAINER" --output "$PILOT_OUTPUT" \
    --epochs 3 --workers "$workers" \
    --max-samples "$maximum_samples" --max-steps "$maximum_steps" \
    2>&1 | tee "$LOG_ROOT/winner_sequence_correction_pilot.log"
  exit "${PIPESTATUS[0]}"
fi
...
if [[ "$stage" == "pilot-acceptance" ]]; then
  "$PYTHON" "$AUDITOR" \
    ...
    2>&1 | tee "$LOG_ROOT/winner_sequence_correction_pilot_acceptance.log"
  exit "${PIPESTATUS[0]}"
fi
...
if [[ "$stage" == "acceptance" ]]; then
  "$PYTHON" "$AUDITOR" \
    ...
    --batch-size 64 --workers "$workers" --device cuda \
    2>&1 | tee "$LOG_ROOT/winner_sequence_correction_acceptance.log"
  exit "${PIPESTATUS[0]}"
fi
...
"$PYTHON" "$TRAINER" --output "$FORMAL_OUTPUT" \
  ...
  2>&1 | tee "$LOG_ROOT/winner_sequence_correction_training.log"
exit "${PIPESTATUS[0]}"
